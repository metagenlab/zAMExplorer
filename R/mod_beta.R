...
zamp_ordination <- function(ps, rank, method, distance, transform, colour_var, shape_var = "None", ellipse = TRUE) {
  ...
  needed <- colour_var
  if (!identical(shape_var, "None")) needed <- unique(c(needed, shape_var))
  keep <- stats::complete.cases(meta[, needed, drop = FALSE])
  mat <- mat[keep, , drop = FALSE]
  meta <- meta[keep, , drop = FALSE]
  shiny::validate(shiny::need(nrow(mat) >= 3, "At least three samples with complete plotting metadata are required for ordination."))

  if (distance == "aitchison") transform <- "clr"
  if (distance == "jaccard") transform <- "binary"
  x <- zamp_transform_matrix(mat, transform)
  x <- x[, vapply(as.data.frame(x), function(v) stats::sd(v, na.rm = TRUE) > 0, logical(1)), drop = FALSE]
  shiny::validate(shiny::need(ncol(x) >= 2, "Too few variable taxa remain to calculate a two-dimensional ordination."))

  note <- NULL
  if (method == "PCA") {
    fit <- tryCatch(
      stats::prcomp(x, center = TRUE, scale. = FALSE),
      error = function(e) shiny::validate(shiny::need(FALSE, paste("PCA could not be calculated:", conditionMessage(e))))
    )
    shiny::validate(shiny::need(ncol(fit$x) >= 2, "PCA returned fewer than two axes."))
    ...
    shiny::validate(shiny::need(all(is.finite(as.vector(dist_obj))), "The selected distance contains non-finite values."))

    if (method == "PCoA") {
      fit <- tryCatch(
        stats::cmdscale(dist_obj, k = 2, eig = TRUE, add = TRUE),
        error = function(e) shiny::validate(shiny::need(FALSE, paste("PCoA could not be calculated:", conditionMessage(e))))
      )
      shiny::validate(shiny::need(ncol(fit$points) >= 2, "PCoA returned fewer than two axes."))
      ...
      fit <- tryCatch(
        vegan::metaMDS(dist_obj, k = 2, trymax = 80, autotransform = FALSE, trace = FALSE),
        error = function(e) shiny::validate(shiny::need(FALSE, paste("NMDS could not be calculated:", conditionMessage(e))))
      )
      coords <- vegan::scores(fit, display = "sites", choices = 1:2)
      shiny::validate(shiny::need(ncol(as.matrix(coords)) >= 2, "NMDS returned fewer than two axes."))
      xlab <- "NMDS1"
      ylab <- "NMDS2"
  ...
  p <- ggplot2::ggplot(df, aes_base) +
    ...
    ggplot2::labs(
      ...
      x = xlab, y = ylab, colour = colour_var,
  ...
  if (isTRUE(ellipse)) {
    ...
    if (length(valid)) {
      p <- p + ggplot2::stat_ellipse(
        ...
        level = 0.95, linewidth = 0.65, alpha = 0.8, show.legend = FALSE
      )
    }
  }
...
mod_beta_server <- function(id, ps, output_dir, auto_save) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    ord <- shiny::eventReactive(input$generate, {
      ...
      tryCatch(
        zamp_ordination(ps(), input$rank, input$method, input$distance, input$transform, input$colour, input$shape, input$ellipse),
        error = function(e) shiny::validate(shiny::need(FALSE, paste("Beta-diversity ordination failed:", conditionMessage(e))))
      )
    }, ignoreInit = TRUE)
    ...
    output$plot <- plotly::renderPlotly({
      ...
      tryCatch(
        ...
        error = function(e) shiny::validate(shiny::need(FALSE, paste("Beta-diversity plot could not be rendered:", conditionMessage(e))))
      )
    })
    ...
    beta_stats <- shiny::eventReactive(input$run_stats, {
      ...
      tryCatch({
        ...
        required <- input$stats_group
        if (!identical(input$strata, "None")) required <- unique(c(required, input$strata))
        ...
        shiny::validate(shiny::need(length(group) >= 3, "At least three complete samples are required."))
        shiny::validate(shiny::need(nlevels(group) >= 2, "At least two groups are required."))
        shiny::validate(shiny::need(all(table(group) > 0), "All retained groups must contain samples."))

        strata <- NULL
        if (!identical(input$strata, "None")) {
          strata <- factor(meta[[input$strata]])
          shiny::validate(shiny::need(nlevels(strata) >= 2, "The selected permutation strata has fewer than two non-missing levels."))
        }
        ...
        dispersion_df <- data.frame(
          ...
          F = c(unname(bd_anova$`F value`[1]), unname(bd_perm$tab[1, "F"])),
          p_value = c(unname(bd_anova$`Pr(>F)`[1]), unname(bd_perm$tab[1, "Pr(>F)"])),
    ...
    shiny::observeEvent(input$generate, {
      if (isTRUE(auto_save())) {
        tryCatch({
          shiny::req(ord_plot())
          file <- zamp_save_plot_to_dir(ord_plot(), output_dir(), paste0("beta_", input$method, "_", input$distance), "pdf", 7, 6)
          shiny::showNotification(paste("Saved:", basename(file)), type = "message")
        }, error = function(e) {
          shiny::showNotification(paste("Beta Diversity ran, but automatic plot saving failed:", conditionMessage(e)), type = "warning", duration = NULL)
        })
      }
    }, ignoreInit = TRUE)
    ...
    shiny::observeEvent(input$run_stats, {
      if (isTRUE(auto_save())) {
        tryCatch({
          ...
          shiny::showNotification(paste("Beta-diversity statistics ran, but automatic table saving failed:", conditionMessage(e)), type = "warning", duration = NULL)
        })
      }
    }, ignoreInit = TRUE)
  })
